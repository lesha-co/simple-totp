...
const writeBits = (buf, beginAtBit, nBits, value) => {
    ...
    if (buf.length * 8 - (beginAtBit + nBits) < 0) {
        throw new Error(`Attempt to write past buffer end: Buffer length: ${buf.length * 8} bits, writing ${nBits} at ${beginAtBit}`);
    }
    value &= 2 ** nBits - 1;
    ...
    buf.writeUInt8((buf.readUInt8(startingByte) | shiftNumber(value, shift)) & 0xff, startingByte);
    if (shift < 0) {
        // next byte
        const offsetInNextByte = offsetInStartingByte - 8;
        const shift = 8 - nBits - offsetInNextByte;
        buf.writeUInt8((buf.readUInt8(startingByte + 1) | shiftNumber(value, shift)) & 0xff, startingByte + 1);
    }
};
exports.decode = (b32string) => {
    if (b32string.length % 8)
        throw new Error("input should have length of multiple of 8");
    if (b32string.includes(alphabet_1.padding)) {
        b32string = b32string.slice(0, b32string.indexOf(alphabet_1.padding));
    }
    const codepoints = Array.from(b32string).map((x) => alphabet_1.alphabet.indexOf(x));
